...
class XPeriodPicker extends PiCoComponent {
    css() {
        return `
                :host{ display:inline-block; font:14px/1.35 system-ui,-apple-system,Segoe UI,Roboto,sans-serif; }
        :host([disabled]){ opacity:.6; pointer-events:none; }
        .field{ position:relative; width: 280px; }
        input{
          width:100%; box-sizing:border-box; padding:8px 64px 8px 10px;
          border:1px solid #cbd5e1; border-radius:10px; background:#fff; color:#0f172a;
        }
        input:focus{ outline:none; border-color:#6366f1; box-shadow:0 0 0 3px #6366f122; }
        .btns{ position:absolute; top:0; right:0; height:100%; display:flex; gap:2px; align-items:center; padding:0 3px; }
        .icon, .clear{
          border:1px solid #e2e8f0; background:#f8fafc; border-radius:8px; width:30px; height:28px; cursor:pointer; color:#475569;
        }
        .icon:hover, .clear:hover{ background:#eef2ff; }
        .popup{
          position:absolute; z-index:9999; margin-top:2px; min-width: 260px;
          background:#fff; color:#0f172a; border:1px solid #e2e8f0; border-radius:12px; box-shadow:0 12px 34px #00000018;
          padding:10px; display:none;
        }
        .popup.open{ display:block; }
        .cal-head{ display:flex; align-items:center; justify-content:space-between; gap:8px; margin-bottom:6px; }
        .nav{ display:flex; gap:6px; }
        .nav button{ border:1px solid #e2e8f0; background:#f8fafc; border-radius:8px; width:28px; height:28px; cursor:pointer; }
        ...
        .day{
          border:0; background:#f8fafc; border-radius:8px; padding:8px 0; cursor:pointer; text-align:center; position:relative;
        }
        .day:hover{ background:#eef2ff; }
        .day[aria-disabled="true"]{ cursor:not-allowed; opacity:.35; }
        .day.range{ background:#e0e7ff; }                    /* in-range preview/final */
    ...
    htm() {
        ...
        <div class="btns">
          ...
          <button class="icon" type="button" title="Apri calendario" aria-label="Apri calendario">🗓️</button>
        </div>
    ...
    _render() {
        ...
        for (let i = 0; i < totalCells; i++) {
            ...
            console.log('d', d, this._iso(d));

            cell.dataset.iso = this._iso(d).replaceAll(' ', '');
